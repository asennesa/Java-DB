...
FROM wizzard_deposits GROUP BY deposit_group ORDER BY longest_magic_wand,deposit_group;
...
# 8. Deposit Charge
SELECT deposit_group,magic_wand_creator,min(deposit_charge) AS min_deposit_charge FROM wizzard_deposits GROUP BY deposit_group,magic_wand_creator
...
ELSE '[61+]' END) AS age_group,COUNT(id) AS wizard_count FROM wizzard_deposits GROUP BY age_group ORDER BY age_group;

# 10.First Letter
Select LEFT(first_name,1) AS first_letter FROM wizzard_deposits
WHERE DEPOSIT_GROUP = 'Troll Chest' GROUP BY FIRST_LETTER ORDER BY first_letter ASC;
